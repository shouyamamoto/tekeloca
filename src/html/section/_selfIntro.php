<section class="selfIntro" id="selfIntro">
    <div class="selfIntro-container">
        <h2 class="section-title">撮影者紹介</h2>

        <div class="selfIntro-inner">
            <div class="selfIntro-inner__top">
                <img src="/tekeloca/dist/images/me.jpg" class="mePic">

                <div class="selfIntro-inner__topRight">
                    <p class="name">山本 翔</p>
                    <p class="place">兵庫県出身 大阪在住</p>
                </div>
                
            </div>
            <div class="selfIntro-inner__bottom">
                <p class="selfIntro-inner__bottom-text">何気ない日々の写真を残したいと思い、カメラを始める。休日はカメラを持って彼女と風景や季節の花などを撮ることが楽しみ。<br>
                好きな食べ物は抹茶。</p>
                <p class="selfIntro-inner__bottom-text">インスタグラムはこちら<a href="https://www.instagram.com/ymsy_1113/">https://www.instagram.com/ymsy_1113/<a></p>
            </div>
        </div>
    </div>
</section>